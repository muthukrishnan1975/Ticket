...
const Modalasset = (props) => {
	...
	// 		const itemData = `${item.CostCentreName.toUpperCase()}`;
				...
				<View>
					...
					/> */}
						<Searchbar
							...
							onChangeText={(value) => props.onChangeSearchAsset(value)}
							value={props.searchasset}
						/>
					...
					/> */}
						<FlatList style={styles.flatlist}
							...
							renderItem={({ item }) => {
								return <TouchableOpacity onPress={() => props.actionOnRowAsset(item)}>
									<Text style={styles.itemTitle}>{item.AssetName}</Text>
								</TouchableOpacity>
							}}
						/>
					</View>
